...
## fullpath (команда, переменная окружения) НЕ ДО КОНЦА 3
    while (env[i] && strnstr(env[i], "PATH=", 5)) // Ищем строку PATH, в которой хранятся адресы команд
		i++;

## child_process (команда, переменная окружения) НЕ ДО КОНЦА 2
Функция распознает команду, строит до нее путь и что-то еще
...
## parent_process (команда, переменная окружения, fd f1) НЕ ДО КОНЦА 1
...
Функция создает трубу, настраивает ее концы, создает дочерний процесс, выполняет cmd 1;
    ...
    Родитель:
        close(end[1]); // закрываем выход
        dup2(end[0], 0); // открываем вход, 



## main (количество параметров, параметры, переменная окружения(инфа о система)) НЕ ДО КОНЦА 4
    f1 - fd infile
    f2 - fd outfile
